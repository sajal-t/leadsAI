...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const db = dbAdmin();
  const [business, calls, script, email, site] = await Promise.all([
    ...
    db.from("generated_emails").select("*").eq("business_id", id).eq("user_id", auth.user.id).order("created_at", { ascending: false }).limit(1),
  ...
  return NextResponse.json({
    ...
    generated_email: email.data?.[0] ?? null,
